...
class WriteDB:
    ...
    def update_db(self):
        self.get_new_json()
        old_json = self.cursor.execute('''SELECT json from schedule WHERE id=1''').fetchone()
        self.cursor.execute('''UPDATE schedule SET json = ? WHERE id=1''', (self.json,))
        self.cursor.execute('''UPDATE schedule SET json = ? WHERE id=0''', (''.join(old_json, )))
    ...
    def check(self, old_json):
        self.get_new_json()
        if int(*loads(self.json)['PERIODS'].keys()) > int(*loads(old_json)['PERIODS'].keys()):
            return True
        return False
...
class GetRasp:
    ...
    def getrasp():
        ...
        schedule_js.encoding = 'utf-8'
        schedule_js = BeautifulSoup(schedule_js.text, "html.parser")
        data_py = loads(str(schedule_js)[133:-3])
        rasp_dict = {}
...
class GetArticle:
    ...
    def saving_images(self):
        ...
        for i in li_names:
            with open('news{}.png'.format(li_names.index(i)), 'w') as image:
                image.truncate(0)
        for i in li_names:
            for j in li_images_links:
                if i == j[0]:
                    response, content = cache.request(j[1])
                    image = open('news{}.jpg'.format(li_names.index(i)), 'wb')
                    image.write(content)
                    image.close()
                    img = Image.open('news{}.jpg'.format(li_names.index(i)))
                    img.save('news{}.png'.format(li_names.index(i)))
                    remove(os.path.abspath('news{}.jpg'.format(li_names.index(i))))
...
class SubMainWidget(MainWidget):
    ...
    def set_logowidget(self):
        ...
        self.logo_widget.setStyleSheet("QWidget#logo_widget{\n"
                                       ...
                                       "    border-image: url(:/images/Logo_L.png);\n"
                                       "}")
...
class AboutWidget(SubMainWidget):
    ...
    def set_lyceumphotowidget(self):
        ...
        self.lyceumPhoto_widget.setStyleSheet("border-image: url(:/images/lyceum.png);"
                                              "border-radius: 13px;")
    ...
    def set_schedulephotowidget(self):
        ...
        self.schedulePhoto_widget.setStyleSheet("border-image: url(:/images/schedule.png);"
                                                "border-radius: 13px;")
    ...
    def set_minobrrfphotowidget(self):
        ...
        self.minobrrfPhoto_widget.setStyleSheet("border-image: url(:/images/minobrrf.png);"
                                                "border-radius: 13px;")
    ...
    def set_minobrnsophotowidget(self):
        ...
        self.minobrnsoPhoto_widget.setStyleSheet("border-image: url(:/images/minobrnso.png);"
                                                 "border-radius: 13px;")
    ...
    def set_itschoolphotowidget(self):
        ...
        self.ITSchoolPhoto_widget.setStyleSheet("border-image: url(:/images/ITSchool.png);"
                                                "border-radius: 13px;")
    ...
    def set_ranschoolphotowidget(self):
        ...
        self.RANschoolPhoto_widget.setStyleSheet("border-image: url(:/images/RANschool.png);"
                                                 "border-radius: 13px;")
...
class Logic(CalendarWidget, News0Widget, News1Widget, News2Widget, News3Widget, News4Widget, News5Widget, AboutWidget,
    ...
    def raspbuttonsclicked(self):
        ...
        if self.FUTURE_FLAG:
            rasp = GetRasp.get_previousrasp(WriteDB().get_old_json())[group]
    ...
    def update_data(self):
        ...
        if WriteDB().check(WriteDB().get_old_json()):
            WriteDB().update_db()
